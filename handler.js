...
//         reject("Both the fields required");
//       }
//       else {
...
module.exports.addtask = async (event) => {
  ...
  let sql = "select txtEmail from tblleads where txtEmail='" + email + "';"
  let sql1 = "insert into tblactivity (Subject,txtComments,dtCreatedOn,txtAssignedto,LeadEmail,Status) values ('" + Subject + "', '" + txtcomments + "','" + dtCreatedOn + "','" + txtAssignedto + "','" + Status + "','" + LeadEmail + "');"
  let result = await new Promise((resolve, reject) => {
...
module.exports.updatesignletask = async (event) => {
  ...
  module.exports.taskfetch = async (event) => {
    ...
    let sql = "select tasktitle,campaign,Status,dtCreatedOn,Leadname,txtAssignedto owner from  tblactivity group by tasktitle;";
    let result = await new Promise((resolve, reject) => {
      con.query(sql, function (err, result) {
        if (err) throw err;
        console.log(result)
